        ...
        >
          <div className="relative group">
            ...
            <div className="relative flex items-center bg-slate-900 border border-slate-700/50 rounded-2xl p-2 pl-6 shadow-2xl focus-within:border-gold transition-all">
              {isSearching ? <Loader2 className="w-5 h-5 text-gold animate-spin" /> : <Search className="w-5 h-5 text-slate-500 group-focus-within:text-gold transition-colors" />}
              <input 
                type="text"
                ...
                onKeyDown={(e) => {
                  if (e.key === 'Enter') {
                    addToHistory((e.target as HTMLInputElement).value);
                  }
                }}
                onChange={async (e) => {
                  ...
                  // 2. Search through local indexed POSTAL_DATA (Priority)
                  ...
                  // 3. LIVE API ENHANCEMENT (Appended after local prioritized results)
                    ...
                    try {
                      ...
                      if (data && data[0] && data[0].Status === 'Success') {
                        data[0].PostOffice.forEach((po: any) => {
                          const existingMatch = results.find(r => r.postalCode === po.Pincode && r.name.toLowerCase() === po.Name.toLowerCase());
                          if (!existingMatch) {
                            results.push({
                              id: `live-in-${po.Name.toLowerCase().replace(/ /g, '-')}`,
                              name: po.Name,
                              type: `Live PO • ${po.Block} • ${po.District}`,
                              postalCode: po.Pincode,
                              path: `/india`,
                              countryName: po.State,
                              isLive: true
                            });
                          }
                        });
                  ...
                  // GLOBAL (USA/UK/etc) LIVE API
                  const usZipMatch = val.match(/^\d{5}$/);
                  const ukPostcodeMatch = val.match(/^[A-Z]{1,2}[0-9][A-Z0-9]?\s?[0-9][A-Z]{2}$/i);
                  
                  if (usZipMatch || ukPostcodeMatch) {
                    const countryCode = usZipMatch ? 'us' : 'gb';
                    const zipCode = (usZipMatch ? usZipMatch[0] : ukPostcodeMatch![0]).replace(/\s/g, '');
              ...
              <button className="bg-gold hover:bg-gold/80 text-midnight px-6 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest transition-colors flex items-center gap-2">
                {t('analyze')} <ArrowRight className="w-3 h-3 stroke-[3]" />
              </button>
            </div>
          </div>
          ...
          {history.length > 0 && !searchResults.length && (
            ...
            >
              <span className="text-[9px] font-black text-slate-600 uppercase tracking-widest self-center mr-2">{t('recentSearches')}:</span>
              {history.map((query, idx) => (
                <button
                  ...
                  className="px-3 py-1.5 rounded-full bg-slate-900 border border-slate-800 text-[10px] font-bold text-gold/80 hover:border-gold/40 hover:text-gold transition-all"
                >
                  {query}
                </button>
              ))}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                {searchResults.filter(r => r.isLive).map((item, idx) => (
                  ...
                  >
                    <Link
                      to={item.path}
                      className="glass-card p-6 block group relative overflow-hidden h-full border-gold/20"
                    >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {favorites.map((item, idx) => (
              ...
              >
                <Link
                  to={item.path}
                  className="glass-card p-5 group flex items-center justify-between border-slate-800 hover:border-gold/30 transition-all"
                >
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-lg bg-slate-900 flex items-center justify-center border border-slate-800 text-slate-500 group-hover:text-gold transition-colors">
                      <MapPin className="w-4 h-4" />
                    </div>
                    <div>
                      <p className="text-sm font-bold text-white group-hover:text-gold transition-colors">{item.name}</p>
                      <p className="text-[9px] text-slate-600 font-bold uppercase tracking-widest">{item.postalCode || 'REGIONAL'}</p>
                    </div>
                  </div>
                  <button 
                    onClick={(e) => {
